      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {workstations.map((ws) => {
            ...
            const taken = takenIds.includes(ws.id) && !active;
            return (
              <button
                key={ws.id}
                disabled={taken}
                onClick={() => { if (!taken) { onSelect(ws.id); onClose(); } }}
                style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  padding: '0.75rem 1rem',
                  borderRadius: '0.5rem',
                  border: `2px solid ${active ? '#3b82f6' : taken ? '#fca5a5' : '#e2e8f0'}`,
                  background: active ? '#eff6ff' : taken ? '#fef2f2' : '#f8fafc',
              ...
              >
                <div>
                  <div style={{ fontWeight: 600, fontSize: '0.9375rem', color: taken ? '#9ca3af' : '#0f172a' }}>
                    {ws.name}
                  </div>
                  {ws.description && (
                    <div style={{ fontSize: '0.8125rem', color: '#64748b', marginTop: 1 }}>
                      {ws.description}
                    </div>
                  )}
                  {taken && (
                    <div style={{ fontSize: '0.75rem', color: '#ef4444', marginTop: 2, fontWeight: 500 }}>
                      Занято другим экраном
                    </div>
                  )}
                </div>
                <div style={{ width: 10, height: 10, borderRadius: '50%', flexShrink: 0,
                  background: active ? '#3b82f6' : taken ? '#ef4444' : 'transparent' }} />
              </button>
            );
          })}
        </div>
